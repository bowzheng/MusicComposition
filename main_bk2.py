...
n_inputs = 130   # midi events (tick[0], pitch[1:128], velocity[129])
n_steps = 20    # time steps
n_hidden_units = 128   # neurons in hidden layer
n_outputs = 130      # next midi events
TINY = 1e-6    # to avoid NaNs in logs


data = np.zeros((len(dataPattern[0]), n_inputs), dtype=np.float)
i = 0
for event in dataPattern[0]:
    data[i][0] = event.tick
    data[i][event.data[0]+1] = 1
    data[i][-1] = event.data[1] > 0
    i = i + 1
...
pitch = np.argmax(y[1:128], axis=0)
pitch_pred = np.argmax(pred[1:128], axis=0)
error1 = 100 * (y[0] - pred[0]) ** 2 + (float(pitch) - float(pitch_pred)) ** 2 + (y[-1] - pred[-1])** 2
error = tf.reduce_mean(error1)
#y_ = np.array([y[0], pitch, y[-1]])
#pred_ = np.array([pred[0], pitch_pred, pred[-1]])
train_op = tf.train.AdamOptimizer(lr).minimize(error)
accuracy = tf.reduce_mean(tf.cast(tf.abs(y - pred) < 0.05, tf.float32))
...
predicted_y = pred.eval(feed_dict = {x: tt_x})[0]
