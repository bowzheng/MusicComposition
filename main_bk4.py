...
def RNN(X, weights, biases):
    ...
    outputs = tf.reshape(outputs, [-1, n_outputs])
    outputs = tf.unpack(tf.transpose(outputs, [1, 0, 2]))    # states is the last outputs
    #outputs = tf.unpack(outputs)

    results = tf.matmul(outputs, weights['out']) + biases['out']    # shape = (128, 10)
...
n_inputs = 1   # midi events (tick[0], pitch[1:128], velocity[129])
n_steps = 32    # time steps
n_hidden_units = 128   # neurons in hidden layer
n_outputs = 1      # next midi events
...
with tf.Session() as sess:
    ...
    while step * batch_size < training_iters:
        ...
        batch_ys = []
        for i in range(batch_size):
            temp = data[i_batch+i+n_steps:i_batch+i+2*n_steps]
        ...
        batch_ys = batch_ys.reshape([batch_size, n_steps, n_outputs])
...
predicted_y = pred_.eval(feed_dict = {x: tt_x})[0][0]
