...
minD = min(data)
maxD = max(data)
outData = [(i - minD)/float(maxD - minD) for i in data]
#outTick = [i/float(480) for i in tick]
#data = [outData, outTick]
#data = [[outTick[i], outData[i], on[i]] for i in range(len(outData))]
data = outData
print data
...
n_hidden_units = 8   # neurons in hidden layer
